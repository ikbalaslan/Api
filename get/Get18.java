...
public class Get18 extends GMIBankBaseUrl {/*
    ...
    @Test
    public void get18(){
        spec.pathParams("f","tp-customers","s",110452);
       ...
       Country country = new Country(3,"USA",null);
        ...
        Response res = given().spec(spec).contentType(ContentType.JSON).headers("Authorization","Bearer "+generateToken()).when().get("/{f}/{s}");
        res.prettyPrint();

       Customer actualData = JsonUtils.convertJsonToJavaObject(res.asString(),Customer.class);
